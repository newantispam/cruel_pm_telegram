...
API_TOKEN = '251131038:AAG_GzTtcHkXDj0jCmr7s9UzAmpci9J6Io4'
		...
		if m.new_chat_participant.id == bot.get_me().id:
			...
			if str(cid) not in chat:
				...
				with open('chat.txt', 'a') as f:
					f.write(str(cid)+"\n")
			bot.send_message(cid, "Hi ty for invite me but now im not work in group ������\nIf you want to see all the cute guys behind the Bot type /credits ������")
			...
			groupid = m.chat.id
			bot.send_message('1063433888', "# DEBUG # " + "Bot got invited to the group " + str(groupname) + "(" + str(groupid) + ")", parse_mode="HTML")
